...
import com.sean.user_service.config.security.SecurityConst
import com.sean.user_service.entity.User
import com.sean.user_service.repo.RoleRepo
import com.sean.user_service.repo.UserDto2
import com.sean.user_service.repo.UserRepo
import com.sean.user_service.repo.YodaRepo
...
import com.sean.user_service.ui.res.ErrorMsgList
...
class UserServiceImpl @Autowired constructor(
    ...
    //@PreAuthorize("@userAccess.test(#uid, \"aaaaaaaaaaaa\")")
    override fun showOne(uid: String): UserRes? {
        val user: User = repo.findByUid(uid) ?: throw CException("Not Found")
//        val user2: List<Array<Any>> = repo.findByUid2(uid) as List<Array<Any>>
        val user2: Array<Any> = repo.findByUid2(uid) as Array<Any>
        val user3: UserDto2? = repo.findByUid3(uid)
        println(user3?.email)
...
//        userRes.add(linkTo<UserController> { showOne(uid) }.withRel("user2"))
        return userRes
    }

    fun showOne2(uid: String): TestUser? {
...
//        userRes.add(linkTo<UserController> { showOne(id) }.withSelfRel())
//        userRes.add(linkTo<UserController> { showOne(uid) }.withRel("user2"))
        return yodaRepo.yodaSayFindUserByUid(askYoda) ?: throw CException("Not Found")
    }
